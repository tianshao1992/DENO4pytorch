...
def load_npy(num=2):
    slides = []
    npy_dir_list = os.listdir('data')  # source ：文件夹路径

    for npy_dir in npy_dir_list:
        if os.path.splitext(npy_dir)[1] == ".npy" and 'LES' in os.path.basename(npy_dir):
            side = os.path.join('data', npy_dir)
            slides.append(side)  # 完整的路径
    ...
    for slide in slides:
        if k > num:
            break
        data.append(np.load(slide).astype(np.float32))
        k += 1
...
if __name__ == "__main__":
    ...
    # 优化算法
    Optimizer = torch.optim.Adam(Net_model.parameters(), lr=learning_rate, betas=(0.7, 0.9), weight_decay=1e-9)
    # Optimizer = torch.optim.AdamW(Net_model.parameters(), lr=learning_rate, weight_decay=1e-8)
    # 下降策略
