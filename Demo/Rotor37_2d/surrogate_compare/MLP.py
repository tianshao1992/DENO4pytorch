...
def get_pred(npz_path, n_train, parameter):
    ...
    for epoch in range(100):
        loss = train(train_loader, Net_model, Loss_func, Optimizer, Scheduler)
        if epoch > 0 and epoch % 10 == 0:
            print("epoch: {},  loss: {}".format(epoch, loss))

    Net_model.eval()
    pred = Net_model(valid_x)

    test_results = np.mean(np.abs((valid_y - pred) / valid_y).detach().numpy(), axis=0)
...
if __name__ == "__main__":
    ...
    np.savez(os.path.join("data", "MLP.npz"), **dict_all)
