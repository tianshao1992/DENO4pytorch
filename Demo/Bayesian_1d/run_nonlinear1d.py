...
# @Time    : 2023/2/19 23:03
# @Author  : Liu Tianyuan (liutianyuan02@baidu.com)
# @Site    : 
# @File    : run_pinn_Bayesian.py
"""
import torch
...
def f(x):
    return 0.01 * (-108) * torch.sin(6 * x) * (torch.sin(6 * x) ** 2 - 2 * torch.cos(6 * x) ** 2) + 0.7 * torch.tanh(
        torch.sin(6 * x) ** 3)


data = {}
data['x_u'] = torch.linspace(lb, ub, N_tr_u).view(-1, 1)  # 边界条件
data['y_u'] = u(data['x_u']) + torch.randn_like(data['x_u']) * like_std  # 引入均值为0，方差为0.1的数据
data['x_f'] = torch.linspace(lb, ub, N_tr_f).view(-1, 1)
data['y_f'] = f(data['x_f']) + torch.randn_like(data['x_f']) * like_std  # 引入均值为0，方差为0.1的数据
