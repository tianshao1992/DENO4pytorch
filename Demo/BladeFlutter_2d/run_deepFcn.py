...
if __name__ == "__main__":
    ...
    Net_model = DeepONetMulti(3, [4,], 3, [32, ]*5, [32, ]*6).to(Device)
    # 损失函数
    ...
    for epoch in range(epochs):
        ...
        if epoch > 0 and epoch % 10 == 0:
            ...
            for fig_id in range(5):
                # 100 为验证集算例中的网格节点个数
                train_source, valid_source = input[:ntrain, ...][-fig_id], input[ntrain:ntrain + nvalid, ...][-fig_id]
                train_true, valid_true = output[:ntrain, ...][-fig_id], output[ntrain:ntrain + nvalid, ...][-fig_id]
                ...
                Visual.plot_fields_grid(fig, axs, valid_true.numpy(),
                                      valid_pred.reshape(valid_true.shape))
                ...
                Visual.plot_fields_grid(fig, axs, train_true.numpy(),
                                      train_pred.reshape(valid_true.shape))
                # plt.title('Absolute error:  {:.3e}'.format(float(err_rel[fig_id])), fontdict=Visual.font_EN)
